...
Brain.prototype.updateZWaveNodes = function (message) {
    ...
    for (var index = 0; index < message.d.c.nodes.length; index++) {
        node = message.d.c.nodes[index];
        if (node.ready === true) {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_BRAIN, 'Node ready:', node.id);
            this.insertOrUpdateNode(homeId, node);
        }
...
Brain.prototype.insertOrUpdateNode = function(homeId, node) {
    ...
    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_BRAIN, 'Searching for node in database:', node.id);
    ...
    self.zwaveNodesDb.findOne({ homeId: homeId, nodeId: node.id }, {}, function (err, record) {
        if (err != null) {
            ...
            if (record != null && typeof record != typeof undefined) {
                bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_BRAIN, 'Found node in database:', node.id);
                ...
                self.zwaveNodesDb.insert({ homeId: homeId, nodeId: node.id, node: node }, {}, function (err, result) {
                    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_BRAIN, 'Did not find node in database:', node.id);
