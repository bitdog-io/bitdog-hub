...
Brain.prototype.updateZWaveNodes = function (message) {
    ...
    for (var index = 0; index < message.d.c.nodes.length; index++) {
        ...
        if (node.ready === true) {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_BRAIN, 'Saving node:', node);
            self.zwaveNodesDb.findOne({ homeId: homeId, id: node.id }, {}, function (err, record) {
                if (err != null) {
                    ...
                    if (record != null && typeof record != typeof undefined) {
                        self.zwaveNodesDb.update({ homeId: homeId, id: node.id }, { homeId: homeId, id: node.id, node: node }, function (err, result) {
                            if (err != null) {
                        ...
                        self.zwaveNodesDb.insert({ homeId: homeId, id: node.id, node: node }, {}, function (err, result) {
                            if (err != null) {
