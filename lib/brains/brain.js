...
Brain.prototype.onProcessMessage = function (message) {
    ...
    if (message.h.n === 'bd-zValueChanged') {
        var key = message.h.d.homeId + '-' + message.h.d.valueId;

        self.zwaveValuesDb.find({ id: key }, {}, function (err, result) {
            if (err != null) {
                bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_BRAIN, 'Error finding data in zwaveValuesDb', err);
            }
            else {
                if (result != null && result[0] !== null) {
                    self.zwaveValuesDb.update({ _id: result[0]._id }, { id: key, value: message.h.d.value, date: Date.now() }, function (err, result) {
                        if (err != null) {
