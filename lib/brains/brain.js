    ...
    if (message.h.n === 'bd-zValueChanged') {
        ...
        self.zwaveValuesDb.findOne({ id: key }, {}, function (record) {
            if (record != null && typeof record != typeof undefined) {
                self.zwaveValuesDb.update({ _id: record._id }, { id: key, value: message.d.value, date: Date.now() }, function (err, result) {
                    if (err != null) {
                ...
                self.zwaveValuesDb.insert({ id: key, value: message.d.value, date: Date.now() }, {}, function (err, result) {
                    if (err != null) {
