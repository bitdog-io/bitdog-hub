...
var VideoStreamer = require('./cameras/videoStreamer.js');
...
function BitdogHub() {
    ...
    bitdogClient.addData('bd-alarmEvent', coreMessageSchemas.alarmEventMessageSchema, false);
    ...
    // Weather Data
    bitdogClient.addData('bd-weatherForecast', coreMessageSchemas.weatherForecastMessageSchema, false);
    
    // Video 
    bitdogClient.addCommand('bd-getVideoSources', coreMessageSchemas.getVideoSourcesMessageSchema, function (message, configuration, logger) {
        return { value: videoManager.getVideoSources() };
    }, undefined, undefined, true);
...
BitdogHub.prototype.stop = function () {
    ...
    if (videoStreamer != null) {
        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Stopping.');
        videoStreamer.stop();
    }
...
BitdogHub.prototype.startVideoService = function () {

    bitdogClient.getVideoHubUrl(function (url) { 
        videoStreamer = new VideoStreamer('0'); videoStreamer.start();
    
    
    },
    function (error) { logger.log(constants.LOG_CATEGORY_STATUS, 'Authorization key may be invalid, please register this node'); },
    function (error) { logger.log(constants.LOG_CATEGORY_ERROR, error); });
