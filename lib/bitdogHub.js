...
function BitdogHub() {
    ...
    if (enableZWave === true) {
        ...
        bitdogClient.addCommand('bd-zGetPollingIntensity', coreMessageSchemas.zwaveGetPollingIntensityMessageSchema, function (message, configuration, logger) {
            logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Get polling intensity command received', message);
            var parsedValueId = parseValueId(message.valueId);
            var result = bitdogZWave.getPollingIntensity(parsedValueId.nodeId, parsedValueId.classId, parsedValueId.instanceId, parsedValueId.indexId);
            return { value: result };
        }, undefined, undefined, true);

        bitdogClient.addCommand('bd-zGetPollingEnabled', coreMessageSchemas.zwaveGetPollingEnabledMessageSchema, function (message, configuration, logger) {
            logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Get polling enabled command received', message);
            var parsedValueId = parseValueId(message.valueId);
            var result = bitdogZWave.isPolled(parsedValueId.nodeId, parsedValueId.classId, parsedValueId.instanceId, parsedValueId.indexId);
            return { value: result };
        }, undefined, undefined, true);
    ...
    bitdogClient.addData('bd-automationScheduled', coreMessageSchemas.automationScheduledMessageSchema, true);
