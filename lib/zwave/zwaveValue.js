...
function ZWaveValue(zwaveValueInfo) {
    ...
    if (_value.decoded === true) {
        _notificationStatus = _value.status;
        _value = _value.name;
    }

    // set the notification status
    zwaveValueInfo.notificationStatus = _notificationStatus;
    ...
    this.__defineGetter__("notificationStatus", function () { return _notificationStatus; });
    ...
    function convertNotificationValueToList() {
        ...
        if (_notificationType !== null) {
            ...
            for (_event in _notificationType.events) {
                _values.push(_notificationType.events[_event].name);
            }
    ...
    this.updateValue = function (zwaveValueInfo) {
        ...
        if (convertedValue.decoded === true) {
            // set the status to the status of the decoded message
            status = convertedValue.status;
            // use the new decoded message as the value instead of the orginal numerical code
            convertedValue = convertedValue.name;
        } else {
            // class 156 Alarm Sensor
            // class 98 Door lock
            // class 118 Lock
            // class 32 basic
        ...
        // Update the original OpenZWave value to fix any value type issues but not if it is a decoded message value now
        if (convertedValue.decoded !== true) {
