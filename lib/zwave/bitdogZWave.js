...
function BitdogZWave(bitdogClient) {
        ...
        if (arg instanceof Object) {
            if (typeof arg.d !== typeof undefined && arg.d !== null) {
                ...
                if (typeof arg.d.nodeId !== typeof undefined && arg.d.nodeId !== null) {
                    ...
                    var valueId = brain.parseValueId(arg.d.valueId);
    ...
    if (typeof networkKey === typeof undefined || networkKey === null || networkKey == '') {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Automatically generating network key', zwaveConfiguration);
    ...
    this.createScene = function (label) {
        return zwave.createScene(label);
    };
    ...
    this.sceneGetValues = function (sceneId) {
        zwave.sceneGetValues(sceneId);
    };
    ...
    function sendZwaveConfigurationMessages() {
        ...
        for (var index = 0; index < zwaveHome.nodes.length; index++) {
            ...
            configurationMessage.d.transactionId = dateTime;
            configurationMessage.d.homeId = zwaveHome.homeId;
            configurationMessage.d.nodeId = node.id;
            configurationMessage.d.node = node;
            bitdogClient.sendMessage(configurationMessage);
            neighbors = zwave.getNodeNeighbors(node.id);
        ...
        setTimeout(sendZwaveSceneConfigurationMessages, 1000);
    }

    function sendZwaveSceneConfigurationMessages() {
        var dateTime = Date.now();
        var scenes = zwave.getScenes();

        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Sending updated zwave scene configurations');

        //for (var index = 0; index < zwaveHome.nodes.length; index++) {
        //    var configurationMessage = coreMessageSchemas.zwaveSceneConfigurationMessageSchema.createMessage(constants.MESSAGE_CLASS_ZWAVE_SCENE_CONFIGURATION);
        //    var node = zwaveHome.nodes[index];
