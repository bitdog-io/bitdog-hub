...
function BitdogZWave(bitdogClient) {
    ...
    var _pollInterval = bitdogClient.configuration.get(constants.ZWAVE_POLL_INTERVAL_CONFIG);
    var _intervalBetweenPolls = bitdogClient.configuration.get(constants.ZWAVE_INTERVAL_BETWEEN_POLLS_CONFIG);

    if (typeof _pollInterval === typeof undefined || _pollInterval === null) {
        _pollInterval = 5000; // 5 seconds
        bitdogClient.configuration.set(constants.ZWAVE_POLL_INTERVAL_CONFIG, _pollInterval);
    }
    
    if (typeof _intervalBetweenPolls === typeof undefined || _intervalBetweenPolls === null) {
        _intervalBetweenPolls = 0; // 0 seconds
        bitdogClient.configuration.set(constants.ZWAVE_INTERVAL_BETWEEN_POLLS_CONFIG, _intervalBetweenPolls);
    }
    ...
    var _zwaveConfiguration = {
        ...
        IntervalBetweenPolls : _intervalBetweenPolls,
        Associate: true,
        UserPath: bitdogClient.configuration.configPath
    };
            ...
            if (typeof arg.d !== typeof undefined && arg.d !== null) {
                ...
                if (typeof arg.d.nodeId !== typeof undefined && arg.d.nodeId !== null) {
                    ...
                    if (zwaveNode !== null) {
                        ...
                        bitdogClient.logger.logFormatted(category, text, newLogObject);
    ...
    zwave.on('node added', function (nodeId) {
        ...
        self.emit('node added', zwaveNode);
    });
    ...
    zwave.on('node removed', function (nodeId) {
       ...
       bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Node removed', { zwaveNode: zwaveNode });
        ...
        self.emit('node removed', zwaveNode);
        ...
        bitdogClient.sendData('bd-zNodeRemoved', coreMessageSchemas.zwaveNodeMessageSchema, function (message) {
            ...
            message.nodeInfo = zwaveNode.getNodeInformation();
        });
    ...
    zwave.on('value added', function (nodeId, commandClassId, zwaveValueInfo) {
        if (zwaveHome.updateValue(nodeId, zwaveValueInfo) === true) {
            bitdogClient.sendData('bd-zValueChanged', coreMessageSchemas.zwaveValueMessageSchema, function (message) {
                ...
                message.units = zwaveValueInfo.units;
                message.new = zwaveValueInfo.new;
            });
    ...
    zwave.on('node ready', function (nodeId, zwaveNodeInfo) {
        ...
        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Node ready', { zwaveNode: zwaveNode.displayName });
        ...
        bitdogBrain.getZWaveNode(zwaveHome.homeId, zwaveNode.id).then(function (record) {
            ...
            if (record != null) {
                ...
                bitdogBrain.insertOrUpdateNode(zwaveHome.homeId, zwaveNode);
            }
    ...
    this.setPollingIntensity = function (nodeId, classId, instanceId, indexId, value) {
        zwave.setPollIntensity(nodeId, classId, instanceId, indexId, value);
    };
    
    this.getPollingIntensity = function (nodeId, classId, instanceId, indexId, value) {
        return zwave.getPollIntensity(nodeId, classId, instanceId, indexId, value);
    };
    
    this.enablePolling = function (nodeId, classId, instanceId, indexId, intensity) {
        return zwave.enablePoll(nodeId, classId, instanceId, indexId, intensity);
    };
    
    this.disablePolling = function (nodeId, classId, instanceId, indexId) {
        return zwave.disablePoll(nodeId, classId, instanceId, indexId);
    };

    this.isPolled = function (nodeId, classId, instanceId, indexId) {
        return zwave.isPolled(nodeId, classId, instanceId, indexId);
    };
    ...
    function saveZwaveConfigurations(transactionId, homeId, nodes, successCallback, errorCallback) {
        ...
        var request = {
            ...
            configuration: JSON.stringify({ nodes: nodes })
        };
            ...
            response.on('end', function () {
                ...
                if (this.statusCode == 200 && responseString.length > 1) {
                    ...
                    if (resultObject.Success === true) {
                        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Saved configurations');
    ...
    function sendZwaveConfigurationMessages() {
        ...
        saveZwaveConfigurations(transactionId, zwaveHome.homeId, zwaveHome.nodes, function () {
            bitdogClient.sendData('bd-zConfigurationUpdate', coreMessageSchemas.zwaveConfigurationUpdateMessageSchema, function (message) {
                message.homeId = zwaveHome.homeId;
