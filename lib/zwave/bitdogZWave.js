...
function BitdogZWave(bitdogClient) {
    ...
    zwave.on('controller command', function (nodeId, state, error, message, help) {
        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_ZWAVE, 'Controller commmand feedback: ', { nodeId: nodeId, message: message, state: state, error: error, help: help });

        bitdogClient.sendData('bd-zControllerMode', coreMessageSchemas.zwaveControllerModeMessageSchema, function (message) {
            message.homeId = zwaveHome.homeId;
            message.nodeId = zwaveValueInfo.nodeId;
            message.mode = zwaveValueInfo.message;
        });
    });
