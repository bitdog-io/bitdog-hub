...
var zwaveDeviceClasses = require('./zwaveDeviceClasses.js');
...
function ZWaveNode(nodeId, zwave) {
    ...
    this.setNodeInformation = function (zwaveNodeInfo, ready) {
        ...
        _typeBasic = decimalToHex(zwave.getNodeSpecific(_id),2);
        _typeSpecific = decimalToHex(zwave.getNodeBasic(_id), 2);

        var result = zwaveDeviceClasses.getClasses(_typeBasic, _typeGeneric, _typeSpecific);

        _typeBasic = result.basic;
        _typeGeneric = result.generic;
        _typeSpecific = result.specific;
