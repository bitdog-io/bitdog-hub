...
Scheduler.prototype.tock = function () {
    ...
    if (this.schedules.dayOfWeek !== this.getDayOfWeek())
        ...
        for (var property in this.schedules) {
            ...
            var time = parseInt(property);
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_AUTOMATION, 'Comparing times', { name: name, time: time, time2: fiveMinutesAgo.unix() });
