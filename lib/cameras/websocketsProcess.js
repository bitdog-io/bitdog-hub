...
function startWebSocket(message) {
    ...
    _wsClient.on('close', function(data)  {
        _wsClientOpen = false;
    ...
    _wsClient.on('error', function (error)  {
        process.send({ name: 'ws-connection-error', error: error });
    });
}
...
function startHttpServer(message) {
    _httpServer = http.createServer(function (req, res) {
        ...
        console.log('Local HTTP MPEG Stream Connected: ' + req.socket.remoteAddress + ':' + req.socket.remotePort);
        ...
        req.on('end', function () {
            console.log('No more data in local HTTP response.');
        });
        ...
        console.log('Local HTTP server listening for video stream on port ' + address + ':' + dynamicPort);
        process.send({ name: 'http-connection-ready', host: address, port: dynamicPort });
    });
    
    _httpServer.on('close', function () {
       
        console.log('Local HTTP server closed');
