...
function startWebSocket(message) {
    ...
    _wsClient.on('message',  function (data) {
        ...
        try {
            ...
            switch (data.name) {
                case 'clientCount':
                    message.settings.enableStream = data.count > 0;
                    process.send({ name: 'ws-connection-sending-video', data: { enableStream: message.settings.enableStream } });
                    break;
                default:
            ...
            process.send({ name: 'ws-connection-error', data: { url: url, error: error, data: data } });
            restartWebSocket();
        }
...
function startHttpServer(message) {
    ...
    _httpServer = http.createServer(function (req, res) {
        ...
        req.on('data', function (data) {
            if (_wsClientOpen === true && message.settings.enableStream === true) {
                try {
