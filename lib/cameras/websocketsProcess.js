...
function startHttpServer(message) {
    var videoStreamerPort = bitdogClient.configuration.get(constants.VIDEO_STREAMER_PORT);
    
    if (typeof videoStreamerPort === typeof undefined)
        videoStreamerPort = 9000;
    ...
    _httpServer = http.createServer(function (req, res) {
        ...
        process.send({ name: 'http-server-ready', host: address, port: dynamicPort });
    });
