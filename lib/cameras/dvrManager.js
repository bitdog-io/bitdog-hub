...
DVRManager.prototype.uploadFiles = function (index) {
    ...
    if (files.length > 0) {
        ...
        var lastAccessInMinutes = now.diff(lastAccess).asMinutes();
        
        if (lastAccessInMinutes < 1) {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER, 'Ignoring file too new, maybe open' + fileName);
            setTimeout(self.uploadFiles.bind(self), 1000, ++index);
            return;
        }
