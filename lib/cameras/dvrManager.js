...
DVRManager.prototype.uploadFiles = function (index) {
    ...
    if (files.length > 0) {
        ...
        if ((lastAccessInMinutes < 2 && fileExt !== '.mp4') || (lastAccessInMinutes < 1 && fileExt !== '.jpg') ) {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER, 'File ' + fileName + ' was last accessed ' + lastAccessInMinutes + ' mintues ago. Ignoring file because its too new, maybe open' + fileName);
            ++index;
