...
DVRManager.prototype.start = function (fileAssetUrl) {
    ...
    setTimeout(this.uploadFiles.bind(this), 60000);
...
DVRManager.prototype.uploadFiles = function (index) {
    ...
    if (files.length > 0) {
        ...
        var now = moment();
        var lastAccess = moment(stat.mtime);
        var lastAccessInMinutes = now.diff(lastAccess, 'minutes',true);
        
        if ((lastAccessInMinutes < 2 && fileExt === '.mp4') || (lastAccessInMinutes < 1 && fileExt === '.jpg') ) {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER, 'File ' + fileName + ' was last accessed ' + lastAccessInMinutes + ' mintues ago. Ignoring file because its too new, maybe open');
            ++index;

            if (index === files.length)
                setTimeout(self.uploadFiles.bind(self), 60000, 0);
            else
                setTimeout(self.uploadFiles.bind(self), 1000, index);
            ...
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER, 'File ' + fileName + ' was last accessed ' + lastAccessInMinutes + ' minutes ago');
        }

        self.uploadFile(fileName,stat).then(function (result, error) {
            
            if (typeof error !== typeof undefined && error !== null) {
                bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER,'Error uploading file', error);
...
DVRManager.prototype.uploadFile = function (fileName, stat) {
    var self = this;
    ...
    var promise = new Promise(function (resolve, reject) {
        try {
            ...
            var  formData  =  {
                ...
                fileDateTimeUTC: moment(stat.ctime).toISOString()
            };
            
            request.post({ url: self.url + uri ,  formData:  formData, headers: { node_id: bitdogClient.configuration.nodeId, auth_key: bitdogClient.configuration.authKey } }, function  (error, httpResponse, body)  {
                ...
                try {
                    if  (error !== null)  {
                        ...
                        if (result !== null && result.Success === true ) {
                            ...
                            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER, 'File upload failed for ' + filePath, body);
                            reject(error);
                        }
                    }
                }
