...
DVRManager.prototype.uploadFiles = function (index) {
    ...
    if (files.length > 0) {
        ...
        var now = moment();
        var lastAccess = moment(stat.atime);
        var lastAccessInMinutes = now.diff(lastAccess, 'minutes',true);
        
        if (lastAccessInMinutes < 1) {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER, 'File ' + fileName + ' was last accessed ' + lastAccessInMinutes + ' ago. Ignoring file because its too new, maybe open' + fileName);
            setTimeout(self.uploadFiles.bind(self), 1000, ++index);
            return;
        } else {
            bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_DVR_MANAGER, 'File ' + fileName + ' was last accessed ' + lastAccessInMinutes + ' ago');
        }
