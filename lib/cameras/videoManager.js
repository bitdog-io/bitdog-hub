...
VideoManager.prototype.startVideoServices = function () {
    ...
    if (videoSources.length > 0) {
        logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_MANAGER, 'Found ' + videoSources.length + ' video sources.');
        ...
        bitdogClient.getVideoHubUrl(
            function (url) {
                ...
                for (var index = 0; index < videoSources.length; index++) {
                    ...
                    this.videoStreamers.push(videoStreamer);

                    videoStreamer.start();
                }
            },
        function (error) {
                logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_MANAGER, 'Could not contact video hub.', error);
            },
        function (error) {
                logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_MANAGER, 'Could not contact video hub.', error);
            }
        ...
        logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_MANAGER, 'Did not find any video sources');
    }
};
