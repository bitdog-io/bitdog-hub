...
VideoStreamer.prototype.start = function () {
    ...
    this.websocketProcess.send({ name: 'start', source: this.source, url: this.url + '/source/' + encodeURI(this.source), nodeId: bitdogClient.configuration.nodeId, authKey: bitdogClient.configuration.authKey });
    
}

VideoStreamer.prototype.startEncoder = function (url) {
    var self = this;
    
    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Starting ffmpeg stream to', url);
    
    this.ffmpegProcess = childProcess.spawn('ffmpeg' , ['-s', '640x480', '-f', 'video4linux2', '-i', '/dev/' + self.source, '-f', 'mpeg1video', '-b:v', '800k', '-r', '30', url, '-f','h264', '-f','segment','-segment_time','300','-strftime','1', 'dvr/%Y-%m-%d_%H-%M-%S.mp4']);
    ...
    this.ffmpegProcess.stderr.on('data', function (data) {
       // bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'ffmpeg process error ' + data);
       // ffmpeg puts out a log of messages on stderr that can be ignored, its shouldn't be logged at this point.
    });
