...
function VideoStreamer(sourceId) {
    var _websocketProcess = null;
    var _ffmpegProcess = null;
    var _source = 'video' + parseInt(sourceId);
    var _stopping = false;
...
VideoStreamer.prototype.start = function () {
        ...
        if (typeof message != typeof undefined && typeof message.name != typeof undefined && message.name != null) {
            ...
            switch (message.name) {
                case 'http-connection-ready': {
                    ...
                    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'HTTP connection ready for video', url);

                    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Starting ffmpeg stream to', url);
               
                    this.ffmpegProcess = childProcess.spawn('ffmpeg' , ['-s', '640x480', '-f', 'video4linux2', '-i', '/dev/' + self.source, '-f', 'mpeg1video', '-b:v', '800k', '-r', '30', url]);
                ...
                case 'http-connection-lost':
                    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'HTTP connection lost for video');
                    break;
                case 'ws-connection-lost':
                    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Websocket connection lost for video');
                    break;
                case 'ws-connection-ready':
                    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Websocket connection ready for video');
                    break;
                case 'ws-connection-retry':
                    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Websocket connection reconecting for video');
                    break;
            }
        }
    ...
    this.websocketProcess.send({ name: 'start', source: this.source, url: 'wss://hub.bitdog.io/api/video/source/' + this.source, nodeId: bitdogClient.configuration.nodeId, authKey: bitdogClient.configuration.authKey });
