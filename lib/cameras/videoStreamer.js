...
VideoStreamer.prototype.startEncoder = function (url) {
    ...
    this.ffmpegProcess = childProcess.spawn(process.cwd() + '/bin/startffmpegstream' , [url]);
    
    this.ffmpegProcess.stdout.on('data', function (data) {
        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'ffmpeg process - ' + data);
    });
