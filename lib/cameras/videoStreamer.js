...
VideoStreamer.prototype.start = function () {
    ...
    this.websocketProcess.send({ name: 'start', source: this.source, url: this.url + '/source/' + encodeURI(this.source), nodeId: bitdogClient.configuration.nodeId, authKey: bitdogClient.configuration.authKey, videoStreamerPort: bitdogClient.configuration.get(constants.VIDEO_STREAMER_PORT) });
