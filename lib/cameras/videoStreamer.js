...
function VideoStreamer(source) {
    ...
    bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Starting websocket streamer to Bitdog Cloud for video ' + source);
    var websocketProcess = childProcess.fork(websocketsProcessPath, [], { execArgv: [], cwd: process.cwd(), silent: true });
    
    websocketProcess.stdout.on("data", function (data) {
        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, data);
    });
    
    websocketProcess.stderr.on("data", function (data) {
        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, data);
    });
    
    websocketProcess.on('error', function (error) {
        bitdogClient.logger.logProcessEvent(constants.LOG_PROCESS_VIDEO_STREAMER, 'Websocket streamer error', error);
    });
    
    websocketProcess.on('message', function (message) {
        ...
        if (typeof message != typeof undefined && typeof message.name != typeof undefined && message.name != null) {
            ...
            switch (message.name) {
                case 'connection-ready': {
                    ...
                    var sourceInt = parseInt(source);

                    var ffmpegProcess = childProcess.spawn('ffmpeg' , ['-s', '640x480', '-f', 'video4linux2', '-i', '/dev/video' + sourceInt, '-f', 'mpeg1video', '-b:v', '800k', '-r', '30', url]);
    ...
    websocketProcess.on('disconnected', function () { 
    
    });
    
    websocketProcess.send({ name: 'start', source: source, url: 'wss://hub.bitdog.io/api/video/source/' + source, nodeId: bitdogClient.configuration.nodeId, authKey: bitdogClient.configuration.authKey });
