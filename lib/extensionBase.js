...
ExtensionBase.prototype.__defineSetter__('bitdogHub', function (value) {
    ...
    this._bitdogHub.on('system event', function (eventInfo, configuration, logger) {
        self.processSystemEvent(eventInfo, configuration, logger);
    });
});

ExtensionBase.prototype.__defineGetter__('configuration', function () { return this._bitdogHub.bitdogClient.configuration; });

ExtensionBase.prototype.__defineGetter__('constants', function () { return this._bitdogHub.bitdogClient.constants; });
...
ExtensionBase.prototype.onSystemEvent = function (eventInfo, configuration, logger) {
};
...
ExtensionBase.prototype.processMessage = function (message, configuration, logger) {
    try {
        ...
        logger.logProcessEvent('Bitdog Hub', 'Process message exception: ', error);
    }
};

ExtensionBase.prototype.processSystemEvent = function (eventInfo, configuration, logger) {
    try {
        this.onSystemEvent(eventInfo, configuration, logger);
    }
    catch (error) {
        logger.logProcessEvent('Bitdog Hub', 'Process system event exception: ', error);
    }
};
