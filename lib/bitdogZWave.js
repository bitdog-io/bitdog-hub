...
function BitdogZWave(bitdogClient) {
    ...
    zwave.on('value added', function (nodeId, commandClassId, zwaveValueInfo) {
        zwaveHome.updateValue(nodeId, zwaveValueInfo);
    });
    
    zwave.on('value changed', function (nodeId, commandClassId, zwaveValueInfo) {
        if (zwaveHome.updateValue(nodeId, zwaveValueInfo) === true) {
            bitdogClient.sendData('bd-valueChanged', coreMessageSchemas.zwaveValueMessageSchema, function (message) {
                message.d.homeId = zwaveHome.homeId;
                message.d.valueId = zwaveValueInfo.value_id;
                message.d.value = zwaveValueInfo.value;
            });
        }
    });
